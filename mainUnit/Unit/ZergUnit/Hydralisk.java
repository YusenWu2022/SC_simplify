...
public final class Hydralisk extends ZergUnit implements Buriable, LightArmor {
    public static Image img;
    static {// 经过仔细考虑，还是放在前面的static处理为好，这样就可以避免在每一次创建的时候都得去读取图片浪费大量时间
        try {
            img = ImageIO.read(new File("SC2_simplify/chartlet/hydralisk.jpg"));
        } catch (IOException e) {
            System.out.println("fuck");
        }
    }

    public Hydralisk(int placex, int placey, int side, double speedAmount, double collideVolume, WorldRunner wr) {
        super(85, placex, placey, 14, 40, 0, side, speedAmount, collideVolume, wr);
        setName("hydralisk");
    }
